      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8">
        ...
        <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          >
            ...
            <p className="text-3xl font-bold">{compLoading || allowLoading ? "—" : fmt(grossMonthly, currency)}</p>
          ...
          <div className="rounded-2xl border border-border bg-white p-5 shadow-sm">
            ...
            <p className="text-2xl font-bold text-foreground">{compLoading ? "—" : fmt(baseSalary, currency)}</p>
          ...
          <div className="rounded-2xl border border-border bg-white p-5 shadow-sm">
            ...
            <p className="text-2xl font-bold text-foreground">{allowLoading ? "—" : fmt(totalAllowances, currency)}</p>
        ...
        <div className="mb-6 rounded-2xl border border-border bg-white shadow-sm">
          ...
          <div className="overflow-x-auto">
            {allowLoading ? (
              <p className="px-5 py-6 text-sm text-muted-foreground md:px-6">Loading…</p>
            ) : allowances.length === 0 ? (
              ...
              <table className="w-full min-w-[400px]">
                <thead>
                  <tr className="bg-muted/40">
                    ...
                    <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-muted-foreground">Monthly Amount</th>
                  </tr>
                </thead>
                <tbody>
                  {allowances.map((a, i) => (
                    <tr key={a.name} className="border-t border-border">
                      ...
                      <td className="px-4 py-4 text-right text-sm font-semibold text-foreground">{fmt(a.amount, currency)}</td>
                    </tr>
                  ))}
                  <tr className="border-t border-border bg-muted/20">
                    <td className="px-5 py-3.5 text-sm font-semibold text-foreground md:px-6">Total</td>
                    <td className="px-4 py-3.5 text-right text-sm font-bold text-foreground">{fmt(totalAllowances, currency)}</td>
                  </tr>
                </tbody>
              </table>
            )}
                  ...
                  {payslips.map((p, i) => {
                    ...
                    return (
                      <tr key={p.id} className="hover:bg-muted/20 transition-colors">
                        <td className="px-5 py-3.5 text-sm font-medium text-foreground md:px-6">{periodLabel(p.period)}</td>
                        <td className="px-4 py-3.5 text-right text-sm text-muted-foreground">{fmt(p.baseSalary, p.currency)}</td>
                        <td className="px-4 py-3.5 text-right text-sm text-muted-foreground">{fmt(p.allowances, p.currency)}</td>
                        <td className="px-4 py-3.5 text-right text-sm font-semibold text-foreground">{fmt(p.gross, p.currency)}</td>
