      ...
      <div className="w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        >
          ...
          <div className="relative flex items-start justify-between">
            <div>
              ...
              <p className="mt-0.5 text-lg font-bold">{periodLabel(p.period)}</p>
              <div className="mt-2.5">
                <p className="text-xs font-semibold">{user?.name ?? "—"}</p>
                <p className="text-xs text-white/70">{user?.email ?? ""}</p>
              </div>
            </div>
            <button onClick={onClose} className="rounded-lg p-1 text-white/70 hover:bg-white/20">
              <X className="size-5" />
            </button>
          </div>
          ...
          <div className="relative mt-3 flex items-end justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">{money(p.netPay)}</p>
            </div>
            <div className="text-right text-xs text-white/70">
              <p>Effective deduction rate</p>
              <p className="text-sm font-semibold text-white">{er.toFixed(1)}%</p>
            </div>
          </div>
        </div>
        ...
        <div className="max-h-[60vh] overflow-y-auto p-5">
          <div className="grid grid-cols-2 gap-5">
            {/* Earnings */}
            <div>
              <p className="mb-2.5 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Earnings</p>
              <div className="flex flex-col gap-1.5">
                {earnings.map((e, i) => (
                  <div key={i} className="flex items-center justify-between">
                    <span className="text-xs text-foreground">{e.label}</span>
                    <span className="text-xs font-medium text-foreground">{money(e.amount)}</span>
                  </div>
                ))}
                <div className="mt-1 flex items-center justify-between border-t border-border pt-1.5">
                  <span className="text-xs font-semibold text-foreground">Gross</span>
                  <span className="text-xs font-bold text-foreground">{money(p.gross)}</span>
                </div>
              </div>
            </div>
            ...
            {/* Deductions */}
            <div>
              <p className="mb-2.5 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Deductions</p>
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-foreground">Tax &amp; Statutory</span>
                  <span className="text-xs font-medium text-[#dc2626]">-{money(p.deductions)}</span>
                </div>
                <div className="mt-1 flex items-center justify-between border-t border-border pt-1.5">
                  <span className="text-xs font-semibold text-foreground">Total</span>
                  <span className="text-xs font-bold text-[#dc2626]">-{money(p.deductions)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 flex items-center justify-between rounded-xl bg-primary/5 px-3 py-2.5">
            <span className="text-xs font-semibold text-foreground">Net Pay</span>
            <span className="text-lg font-bold text-primary">{money(p.netPay)}</span>
          </div>
          ...
          <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
            ...
            <span className={cn("rounded-full px-2 py-1 font-semibold", statusBadge[p.status])}>
        ...
        <div className="border-t border-border px-5 py-3">
          <button
            onClick={onClose}
            className="w-full rounded-lg border border-border py-2 text-xs font-medium text-foreground hover:bg-muted"
          >
            Close
          </button>
        </div>
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="border-b border-border bg-white px-3 py-2.5 lg:hidden">
        <h1 className="text-sm font-semibold text-foreground">Payslips</h1>
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-5 lg:p-6">
        ...
        {loading ? (
          <p className="text-xs text-muted-foreground">Loading payslips…</p>
        ) : error ? (
          ...
          <div className="rounded-2xl border border-border bg-white p-6 text-center shadow-sm">
            <p className="text-xs font-medium text-foreground">No payslips yet</p>
          ...
          <>
            ...
            <div className="mb-5 grid grid-cols-2 gap-2.5 sm:grid-cols-4">
              <div
                className="relative col-span-2 overflow-hidden rounded-2xl p-4 text-white sm:col-span-1"
                style={{ background: "linear-gradient(135deg, #5A7CFF 0%, #3B5BDB 100%)" }}
              >
                <span className="absolute -right-4 -top-4 size-20 rounded-full bg-white/10" />
                <p className="text-xs text-white/70">YTD Net Pay</p>
                <p className="mt-0.5 text-xl font-bold">{fmt(ytdNet, currency)}</p>
                <p className="mt-1 text-[11px] text-white/70">{ytd.length} payslips in {thisYear}</p>
              </div>

              <div className="rounded-2xl border border-border bg-white p-3 shadow-sm">
                <div className="mb-1.5 flex size-8 items-center justify-center rounded-lg bg-primary/10">
                  <BadgeDollarSign className="size-4 text-primary" />
                </div>
                <p className="text-xs text-muted-foreground">YTD Gross</p>
                <p className="mt-0.5 text-base font-bold text-foreground">{fmt(ytdGross, currency)}</p>
              </div>

              <div className="rounded-2xl border border-border bg-white p-3 shadow-sm">
                <div className="mb-1.5 flex size-8 items-center justify-center rounded-lg bg-rose-100">
                  <Receipt className="size-4 text-rose-500" />
                </div>
                <p className="text-xs text-muted-foreground">YTD Deductions</p>
                <p className="mt-0.5 text-base font-bold text-foreground">{fmt(ytdDeduct, currency)}</p>
              </div>

              <div className="rounded-2xl border border-border bg-white p-3 shadow-sm">
                <div className="mb-1.5 flex size-8 items-center justify-center rounded-lg bg-emerald-100">
                  <Wallet className="size-4 text-emerald-600" />
                </div>
                <p className="text-xs text-muted-foreground">vs Last Month</p>
                {delta === null ? (
                  <p className="mt-0.5 text-base font-bold text-foreground">—</p>
                ) : (
                  <>
                    <p className={cn(
            ...
            <div className="mb-1.5 flex items-center justify-between">
              <h2 className="text-sm font-semibold text-foreground">All Payslips</h2>
              ...
              {payslips.map((p, i) => {
                ...
                return (
                  <div
                    key={p.id}
                    className={cn(
                      "group relative cursor-pointer overflow-hidden rounded-2xl border bg-white p-4 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5",
                  ...
                  >
                    ...
                    {/* Period */}
                    <p className="text-xs font-semibold text-foreground">{periodLabel(p.period)}</p>

                    {/* Net pay */}
                    <p className="mt-1.5 text-xl font-bold text-foreground">{fmt(p.netPay, p.currency)}</p>
                    ...
                    <div className="mt-3 flex items-center gap-1.5">
                      ...
                      <span className={cn("rounded-full px-2 py-1 text-[10px] font-semibold", statusBadge[p.status])}>
                        {p.status === "PAID" ? "Paid" : p.status === "PROCESSING" ? "Processing" : "Pending"}
                      </span>
                    </div>
                  </div>
